...
unsigned enum_decode
  ...
  for (d = 0; d <= (1 << (N - M)) - 1; d++) {
    ...
    switch (G->type) {
      case Sparse: {
        sparse_encode(sblk, cblk, H, G->L, G->U, G->row_ordering, G->column_ordering);
        break;
      }
      case Dense: {
        dense_encode(sblk, cblk, G->G, G->row_ordering, G->column_ordering, u, v);
        break;
      }
      case Mixed: {
        mixed_encode(sblk, cblk, H, G->G, G->row_ordering, G->column_ordering, u, v);
        break;
      }
    }
